<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<SMCHSGManager.Models.EventRegistration>" %>

<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
		SMCH Association Singapore - 	Event Registration Delete
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
<div id="body">
    <h5>Delete Event Registration Confirmation</h5>
      <div class="centerblock">
    <h6>Are you sure you want to cancel the Event <%: Model.Event.Title  %> Registration from 
    <strong> <%: String.Format("{0:g}", Model.Event.StartDateTime) %></strong> to <strong><%: String.Format("{0:g}", Model.Event.EndDateTime)%></strong>?</h6>
    
    <% using (Html.BeginForm()) { %>
              <div class="editbuttons"  align="center">
		            <input type="submit" value="Delete"  class ="buttonsmall"/>  &nbsp;&nbsp;&nbsp;&nbsp;
                    <%: Html.ActionLink("Cancel", "Details", new { id = Model.ID}, new { @style = "color:white;", @class = "buttonsmall" })%>
             </div>
    <% } %>

    </div>
</div>
</asp:Content>
