 ...
 <div class="fullwidth">    
	...
	<div class="mvcMembership-account">
		<dl class="mvcMembership">
			...
			<% }else{ %>
			<dt>Last Active:</dt>
				<dd><% =Model.User.LastActivityDate.ToUniversalTime().AddHours(8).ToString("MMMM dd, yyyy h:mm:ss tt", CultureInfo.InvariantCulture)%></dd>
			<dt>Last Login:</dt>
				<dd><% =Model.User.LastLoginDate.ToUniversalTime().AddHours(8).ToString("MMMM dd, yyyy h:mm:ss tt", CultureInfo.InvariantCulture)%></dd>
			<% } %>
			<dt>Created:</dt>
				<dd><% =Model.User.CreationDate.ToUniversalTime().AddHours(8).ToString("MMMM dd, yyyy h:mm:ss tt", CultureInfo.InvariantCulture)%></dd>
		</dl>
		...
		<% using(Html.BeginForm("ChangeApproval", "UserAdministration", new{ id = Model.User.ProviderUserKey })){ %>
			...
			<input type="submit" value='<% =(Model.User.IsApproved ? "Unapprove" : "Approve") %> Account'  class ="buttonsmall"/>
		<% } %>

       <%-- <% =Html.ActionLink("Roles", "UsersRoles", "UserAdministration", new{area = "UserAdministration", id = Model.User.ProviderUserKey}, null) %>--%>

        <%--<% using (Html.BeginForm("DeleteUser", "MemberInfo", new { area = "", id = Model.User.ProviderUserKey }))
           { %>
			<input type="submit" value="Delete Account" class ="buttonsmall"/>
		<% } %>--%>
	...
	<%--	<fieldset>--%>
			...
			<input type="submit" value="Save Email Address and Comments" class ="buttonsmall"/>
	<%--	</fieldset>--%>
		<% } %>
	</div>
